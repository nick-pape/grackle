...
The Grackle server runs claws the way an operating system runs processes. It owns the **mechanism** — spawn, signal, supervise, schedule, route — and nothing about the work itself. Which agent runs, what task it's handed, when it's done: that's **policy**, and policy lives above the kernel. The kernel doesn't decide. It carries out.
...
| **signal**    | Deliver a message to a running or dead-but-resumable claw           | What the signal means, who reacts            |
...
| **route**     | Carry messages between claws over named streams and pipes           | The shape of the conversation                |

The kernel is small on purpose. Everything that decides — the task DAG, the review gate, the scheduler — is a [plugin](../extending/plugins) layered on top. Pull the plugins and you're left with the bare primitives: claws on wires, signals between them, nothing telling them what to do.
...
### Stop a claw — SIGTERM, then SIGKILL

Ask a claw to wind down and it gets a chance to save state first.

```bash
grackle kill <session-id> --graceful
```

Graceful is a **SIGTERM**: the server delivers a `[SIGTERM]` message asking the agent to finish its operation, close its owned fds, and stop — then returns immediately. The claw exits on its own terms. Drop the flag and it's a **SIGKILL**: the session is marked terminal, the host process is disposed, the streams are torn down. No last words.
...
Wires drop. When the transport breaks, the claw goes **suspended** — parked on the server, consuming nothing, full history held. A suspended claw is not a killed one. Reopen the lifecycle stream and it reanimates from where it stood.
...
| `knowledge`     | Derived retrieval and prompt context the claw reads before it perches                         |
