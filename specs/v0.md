...
- **Sidecar**: Lightweight process that runs on each remote environment, wraps an agent runtime via a pluggable adapter, exposes a gRPC service
- **Server**: Central gRPC server on your local machine. Connects to all sidecars, provisions environments, brokers auth tokens, serves a web UI
...
graph TB
    ...
    subgraph CS1["Codespace 1 (managed)"]
        SC1["Sidecar :7433<br/>gRPC + runtime"]
    end

    subgraph CS2["Codespace 2 (managed)"]
        SC2["Sidecar :7433<br/>gRPC + runtime"]
    end

    subgraph Docker["Docker Container (local)"]
        SC3["Sidecar :7433<br/>gRPC + runtime"]
    end
...
Everything is gRPC. The server exposes a single gRPC service. The web UI, the CLI, and potentially future tools are all clients of this service. Communication with sidecars is also gRPC.
...
- **Sidecar** exposes a gRPC service. Server is a gRPC client of each sidecar.
...
### Sidecar Service (what the server calls on each environment)
...
sequenceDiagram
    ...
    Server->>CS: gh cs ssh (bootstrap: install node, sidecar)
    CS-->>Server: bootstrapped
    Server->>CS: gh cs ssh (start sidecar :7433)
...
3. Bootstrap (first time): SSH in, install sidecar, mark bootstrapped
   ```bash
   gh cs ssh -c <name> -- 'bash -s' << 'BOOTSTRAP'
     ...
     npm install -g @grackle/sidecar
     touch ~/.grackle-bootstrapped
   BOOTSTRAP
   ```
4. Start sidecar: `gh cs ssh -c <name> -- 'nohup grackle-sidecar --port 7433 > /tmp/grackle-sidecar.log 2>&1 &'`
...
For containers running on your DevBox (or locally). Same sidecar, different transport.

**Provision flow:**
1. `docker run -d --name grackle-<env-id> -p <local-port>:7433 <sidecar-image>`
...
The sidecar container image is the same environment concept as a codespace (Node, agent runtime dependencies, sidecar binary). Your DevBox runs 1-4 of these.
...
The sidecar doesn't know or care what agent is running. It talks to an `AgentRuntime` interface. Claude Code Agent SDK is the only implementation in V0, but the design supports swapping runtimes without touching the sidecar, server, or any client.
...
1. You configure tokens once locally:
   ...
   ```
2. On provision/connect, server calls `PushTokens` on the sidecar
3. Sidecar writes tokens to expected filesystem locations:
...
stateDiagram-v2
    [*] --> pending: SpawnAgent
    pending --> running: Sidecar starts runtime
...
CREATE TABLE environments (
  ...
  last_seen     TEXT,
  env_info      TEXT,                -- JSON from sidecar GetInfo
...
grackle env add ml-worker --docker --image grackle-sidecar:latest --runtime vercel-ai
...
gantt
    ...
    @grackle/common protos        :w1b, after w1a, 1d
    Sidecar gRPC + mock agent     :w1c, after w1b, 2d
...
2. `@grackle/common` — .proto files, codegen
3. `@grackle/sidecar` — gRPC server, mock agent wrapper (echo)
4. Test: run sidecar, call with `grpcurl`

### Week 2: Server + Adapters
5. `@grackle/server` — SQLite, env registry, adapter manager, gRPC service
...
10. `AgentRuntime` interface + Claude Code adapter in sidecar
...
### Week 4: Clients
15. `@grackle/cli` — all commands
16. `@grackle/web` — Vite + React
17. WS bridge in server
18. Session panel + log replay
19. Env list, spawn dialog, input bar
20. Test: full flow browser → server → sidecar → agent → browser
