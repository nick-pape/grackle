 ...
 */
async function startTaskStub(client: GrackleClient, taskId: string): Promise<string> {
...
test.describe("Scoped session_attach and logs_get", { tag: ["@task"] }, () => {
  ...
  test("orchestrator can session_attach to child session via scoped MCP", async ({
    grackle: { client },
  }) => {
  ...
  test("orchestrator can logs_get from child session via scoped MCP", async ({
    grackle: { client },
  }) => {
    test.setTimeout(60_000);
...
test.describe(
  "workspace_get respects explicit workspaceId under scoped token",
  { tag: ["@task"] },
  () => {
     ...
     */
    test("cross-workspace workspace_get is rejected, not silently redirected to bound workspace", async ({
      grackle: { client },
    }) => {
      test.setTimeout(60_000);
      ...
      const scenario = stubScenario(emitMcpCall("workspace_get", { workspaceId: workspaceBId }));
      await client.orchestration.updateTask({
        id: orchestrator.id as string,
      ...
      expect(
        rejectionResult,
        "cross-workspace workspace_get must be rejected with PERMISSION_DENIED",
      ).toBeDefined();
      // Critically: must NOT contain workspace A's data (that would be the silent redirect bug)
      expect(
        rejectionResult.content,
        "must not silently redirect to bound workspace A",
      ).not.toContain(workspaceAId);
    });
  },
);
