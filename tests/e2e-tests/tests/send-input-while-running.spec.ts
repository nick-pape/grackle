...
import { stubScenario, emitText, emitToolUse, emitToolResult, idle } from "./helpers.js";
...
test.describe("Send input while agent is running", { tag: ["@session"] }, () => {
  test("input field is enabled during active session", async ({ stubTask }) => {
    ...
    await expect(page.getByText("Starting work...", { exact: true })).toBeVisible({
      timeout: 15_000,
    });
    ...
    await expect(page.getByText("You said: test input", { exact: true })).toBeVisible({
      timeout: 10_000,
    });
  });
});
