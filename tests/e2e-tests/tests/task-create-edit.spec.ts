...
test.describe("Unified task create/edit experience", { tag: ["@task"] }, () => {
  test("clicking Create Task opens full-panel form with title and description fields", async ({ appPage, grackle: { client } }) => {
    const page = appPage;
    await createWorkspace(client, "create-panel-proj");
  ...
  test("task creation form has no environment dropdown", async ({ appPage, grackle: { client } }) => {
    const page = appPage;
    await createWorkspace(client, "no-env-proj");
