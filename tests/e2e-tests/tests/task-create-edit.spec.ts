...
test.describe("Unified task create/edit experience", { tag: ["@task"] }, () => {
  ...
  test("creating a task via the panel fills title and description, then navigates to workspace", async ({
    appPage,
    grackle: { client },
  }) => {
    const page = appPage;
    ...
    await page
      .locator('[data-testid="task-edit-description"]')
      .fill("A description with **markdown**");
    ...
    await expect(page.locator('.overviewMarkdown, [class*="overviewMarkdown"]')).toBeVisible({
      timeout: 5_000,
    });
  });

  test("Cancel button in new task form returns to workspace view", async ({
    appPage,
    grackle: { client },
  }) => {
    const page = appPage;
  ...
  test("clicking Edit opens the edit form pre-populated with existing task data", async ({
    appPage,
    grackle: { client },
  }) => {
    const page = appPage;
    ...
    await expect(page.locator('[data-testid="task-edit-description"]')).toHaveValue(
  ...
  test("saving edits updates the task title and description", async ({
    appPage,
    grackle: { client },
  }) => {
    const page = appPage;
    ...
    await expect(page.locator('[data-testid="task-title"]')).toContainText("new-title-task", {
      timeout: 5_000,
    });

    // Updated title should appear on the page
    await expect(page.getByText("new-title-task", { exact: true }).first()).toBeVisible({
      timeout: 5_000,
    });
  });
  ...
  test("task edit form shows dependency multi-select with sibling tasks", async ({
    appPage,
    grackle: { client },
  }) => {
    const page = appPage;
    ...
    await expect(
      page.locator('[data-testid^="dep-option-"]', { hasText: "task-alpha-dep" }),
