...
- **Branch naming**: `{github-username}/{issue}-{feature}` when working on a GitHub issue (e.g., `nick-pape/149-agent-subtask-creation`), or `{github-username}/{feature}` when there's no issue (e.g., `nick-pape/fix-typo-in-readme`).
