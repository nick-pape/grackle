...
- Vitest **emits** unit coverage but no longer self-gates ([#1390](https://github.com/nick-pape/grackle/issues/1390)). Enforcement is a single per-package **combined** gate (see below), not separate unit floors.
- Excluded by default: `src/gen/**` (proto), `src/vendor/**` (vendored AHP), `src/mocks/**`, `*.stories.tsx`, `*.test.{ts,tsx}`.

#### Combined coverage (unit + E2E + Storybook) — the one gate

CI computes a **real repo-wide merged total** that unions Vitest unit + Playwright E2E ([#1383](https://github.com/nick-pape/grackle/issues/1383)) + Storybook ([#1384](https://github.com/nick-pape/grackle/issues/1384)) coverage, and gates each package on its **combined** number ([#1390](https://github.com/nick-pape/grackle/issues/1390)) — there is no separate unit-floor gate.
...
- **Backend E2E coverage** ([#1389](https://github.com/nick-pape/grackle/issues/1389)), same `E2E_COVERAGE=true` flag. The spawned Node server + PowerLine are run with `NODE_V8_COVERAGE` pointed at `tests/e2e-tests/.v8-backend/` (`server-manager.ts`); `stopGrackleStack` awaits each process's clean exit (SIGTERM → exit, SIGKILL fallback) so Node flushes its dump. `globalTeardown` converts the dir via `coverage-options-backend.ts` to `tests/e2e-tests/coverage-backend/lcov.info`, source-mapped to `packages/<pkg>/src` (backend dist already ships source maps via the heft tsconfig-base). **Linux/CI only** — Windows turns SIGTERM into a hard kill, so no dump is written locally.
- **Storybook coverage** ([#1384](https://github.com/nick-pape/grackle/issues/1384)), gated by `STORYBOOK_COVERAGE=true` (set for all CI builds in `.github/actions/build-and-test`). When set, `packages/web-components/.storybook/main.ts` adds `vite-plugin-istanbul` (`forceBuildInstrument`) to instrument the component source; the `storybook-test` heft task runs `test-storybook --coverage` and `StorybookTestPlugin` converts the istanbul result to `packages/web-components/coverage-storybook/lcov.info` (sibling of `coverage/` so the parallel Vitest task's `clean` can't race it). Captures rendering/interaction of story-only components that unit tests miss. The multi-worktree `test-storybook` hang (getProjectRoot() walking to main repo root) is fixed by overriding rootDir/roots/STORYBOOK_STORIES_PATTERN in `StorybookTestPlugin` (#1449).
- **Merge.** `scripts/coverage-merge` (`@grackle-ai/coverage-merge`) unions every `*/coverage*/lcov.info` (unit + E2E browser/backend + Storybook) into one combined lcov + total, normalizing `SF:` paths to repo-relative POSIX so the same file from any suite collapses to one key (a line is covered if any suite hit it). The CI `coverage` job downloads the unit + per-shard E2E artifacts, merges, prints the total + a **per-package unit/e2e/storybook/combined table** to `$GITHUB_STEP_SUMMARY`, and uploads `combined-coverage`. It fails loudly if E2E coverage (`--require-source packages/web/src`) or backend E2E coverage (the `coverage-backend/lcov.info` artifact check) didn't land.
- **Gate** ([#1390](https://github.com/nick-pape/grackle/issues/1390)). Per-package **combined** floors (lines/functions/branches) live in `rigs/heft-rig/combined-thresholds.json`, keyed by `packages/<pkg>` / `scripts/<pkg>`. The merge step runs with `--combined-thresholds` and **fails CI** if any package drops below its floor. Floors are `floor(actual/5)*5` of a real combined run — **frozen** (no auto-ratchet): bump up by hand as coverage climbs, lower by hand if a package proves flaky. A package with combined coverage but no floor entry logs a one-line warning and runs unenforced.
