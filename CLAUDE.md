...
- **Tag every new e2e spec file**: Every `test.describe()` block in `tests/e2e-tests/tests/` must include a Playwright native tag. CI enforces this via `lint-e2e-tags.sh`. Use the syntax `test.describe("Name", { tag: ["@domain"] }, () => {` with one or more tags from: `@task`, `@workspace`, `@environment`, `@session`, `@settings`, `@persona`, `@webui`, `@error`, `@smoke`. This enables selective e2e test running — CI only runs tests relevant to the changed packages.
