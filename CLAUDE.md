...
- **Storybook coverage** ([#1384](https://github.com/nick-pape/grackle/issues/1384)), gated by `STORYBOOK_COVERAGE=true` (set for all CI builds in `.github/actions/build-and-test`). When set, `packages/web-components/.storybook/main.ts` adds `vite-plugin-istanbul` (`forceBuildInstrument`) to instrument the component source; the `storybook-test` heft task runs `test-storybook --coverage` and `StorybookTestPlugin` converts the istanbul result to `packages/web-components/coverage-storybook/lcov.info` (sibling of `coverage/` so the parallel Vitest task's `clean` can't race it). Captures rendering/interaction of story-only components that unit tests miss. CI-only (the multi-worktree `test-storybook` jest-haste-map hang makes local runs unreliable).
