...
describe("Full tool registry", () => {
  it("contains exactly the expected number of tools", () => {
    ...
    expect(registry.list()).toHaveLength(57);
  });
  ...
  it("groups are consistent within tool files", () => {
    ...
    const expectedGroups = new Set([
      "env", "session", "workspace", "task", "finding", "persona", "logs", "credential", "token", "config", "ipc", "usage", "knowledge", "workpad", "schedule", "system",
    ]);
    for (const tool of registry.list()) {
