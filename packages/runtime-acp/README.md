...
| `isolateClaudeConfig` | `boolean`                | `false` | Point `CLAUDE_CONFIG_DIR` at a clean, Grackle-managed dir so the agent does not inherit the developer's personal `~/.claude/settings.json`. Set for Claude-backed bridges (e.g. `claude-code-acp`), whose headless permission-mode enum rejects interactive-only settings such as `permissions.defaultMode: "auto"`. Existing credentials are symlinked (or copied) into the isolated dir so auth is preserved. |
