...
describe("breadcrumb builders", () => {
  ...
  it("task returns Home > Task", () => {
    const task: TaskData = makeTask({ id: "t1", workspaceId: "p1", title: "My Task" });
    const byId: Map<string, TaskData> = new Map([["t1", task]]);

    const segments: BreadcrumbSegment[] = buildTaskBreadcrumbs("t1", byId);
    expect(segments).toHaveLength(2);
    expect(segments[0].label).toBe("Home");
    expect(segments[0].url).toBe("/");
    expect(segments[1].label).toBe("My Task");
    expect(segments[1].url).toBeUndefined();
  });
  ...
  it("nested task includes ancestor chain", () => {
    ...
    const segments: BreadcrumbSegment[] = buildTaskBreadcrumbs("t3", byId);
    expect(segments).toHaveLength(4); // Home > Root > Child > Grandchild
    expect(segments[0].label).toBe("Home");
    expect(segments[1].label).toBe("Root");
    expect(segments[1].url).toBe("/tasks/t1");
    expect(segments[2].label).toBe("Child");
    expect(segments[2].url).toBe("/tasks/t2");
    expect(segments[3].label).toBe("Grandchild");
  ...
  it("new task with parentTaskId shows ancestor chain", () => {
    ...
    const segments: BreadcrumbSegment[] = buildNewTaskBreadcrumbs("t1", byId);
    expect(segments).toHaveLength(3); // Home > Parent > New Task
    expect(segments[0].label).toBe("Home");
    expect(segments[1].label).toBe("Parent");
    expect(segments[2].label).toBe("New Task");
    expect(segments[2].url).toBeUndefined();
  });
