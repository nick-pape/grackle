...
test.describe("Task Lifecycle (stub runtime)", () => {
  test("full task flow: create, start, stream, review, approve", async ({
    appPage,
  }) => {
    const page = appPage;
    ...
    await expect(page.getByText("lifecycle-proj")).toBeVisible({
      timeout: 5_000,
    });
    ...
    await expect(page.getByText("Task: test task")).toBeVisible({
      timeout: 5_000,
    });
    ...
    await expect(page.locator("text=Stub runtime initialized")).toBeVisible({
      timeout: 15_000,
    });
    ...
    await expect(
      page.locator("button", { hasText: "+ New Task" }),
    ).toBeVisible();
  });

  test("task rejection auto-retries and returns to review", async ({
    appPage,
  }) => {
    const page = appPage;
    ...
    await expect(page.getByText("Task: reject task")).toBeVisible({
      timeout: 5_000,
    });
    ...
    const rejectInput = page.locator(
      'input[placeholder="Rejection notes (optional)..."]',
    );
