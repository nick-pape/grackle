...
- Sessions master-detail layout: sidebar nav with status dots, corrected breadcrumbs (Home > Sessions > Session XYZ), Home breadcrumb now navigates to Code view

## 0.193.0
Thu, 11 Jun 2026 21:08:51 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix system task failures on fresh Docker install: retry transient npm install network errors (ENOTFOUND/EAI_AGAIN/ETIMEDOUT) with exponential backoff; surface actionable pre-flight error when a runtime's required credential provider is disabled rather than letting the SDK throw an opaque createSession error.

## 0.192.0
Thu, 11 Jun 2026 19:32:58 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Bump transitive overrides (axios, hono, qs, shell-quote, joi) and react-router direct dep to clear all 19 open Dependabot security alerts

## 0.191.0
Thu, 11 Jun 2026 14:44:32 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Migrate plugin-scheduling, plugin-orchestration, plugin-knowledge, and server to getDatabaseStores() registry (DI migration #1551)
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Extract shared runProvisionLoop primitive from ensureSpawnConnection and provisionEnvironment

## 0.187.0
Thu, 11 Jun 2026 04:57:45 GMT

### Minor changes

- Add `grackle agent` command group (list/create/show/edit/delete) for the new minimal Agent entity (#1417).
- Add `agent edit --heartbeat`, `--heartbeat-clear`, `--heartbeat-rules`, `--heartbeat-rules-file`, `--heartbeat-pause`, `--heartbeat-resume` flags and a heartbeat block in `agent show` (#1438).
- Move AHP orphan-event rescue into the mapper (withTurn helper); document status _meta tunnel as temporary HR8d artifact; delete hardcoded ORPHAN_RESCUABLE_TYPES and STATUS_RESCUE_CONTENTS from PowerLine forwarder
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Introduce RuntimeCapabilities interface on AgentRuntime to express runtime-specific quirks (hook support, resume support, non-empty resume prompt) through types instead of doc comments; add applyMcpConfig() template-method seam on BaseAgentSession for per-SDK MCP config transforms
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Refactor fat UI components: extract data hooks (useTaskPageData, useKnowledgeSidebar, useCoordinationSidebar), split TaskList into focused hooks and subcomponents, move CodespacePicker and DockerContainerPicker to their own files

## 0.184.0
Thu, 11 Jun 2026 02:05:23 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Add unit tests for spawnAgent and provisionEnvironment; extract ensureSpawnConnection helper for testability
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Unify event dispatch async error handling: widen Subscriber to support async, extract webhook-publisher with retry, update PluginContext.subscribe signature

## 0.181.0
Wed, 10 Jun 2026 20:23:55 GMT

### Minor changes

- Add `grackle agent` command group (list/create/show/edit/delete) for the new minimal Agent entity (#1417).
- Add `agent edit --heartbeat`, `--heartbeat-clear`, `--heartbeat-rules`, `--heartbeat-rules-file`, `--heartbeat-pause`, `--heartbeat-resume` flags and a heartbeat block in `agent show` (#1438).
- Add environment re-homing to UpdateAgentRequest (proto optional field), route heartbeat mutations through useAgents hook, and expose editable environment picker in Agent Settings tab
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Add closeDatabase() export for test teardown; add setupTestDatabase() to @grackle-ai/test-utils; migrate 9 handler tests from mock-database to real in-memory SQLite

## 0.178.0
Wed, 10 Jun 2026 16:43:26 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Migrate packages/core from direct store imports to getDatabaseStores() registry

## 0.177.0
Fri, 05 Jun 2026 17:59:37 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Tighten weak test assertions (toBeTruthy/toBeDefined) with specific value checks across 28 test files

## 0.176.0
Fri, 05 Jun 2026 14:37:58 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Standardize error messages: switch require-helpers to GrackleError subclasses, add requireOneOf, fix wrong gRPC codes, log error context in interceptor

## 0.175.0
Fri, 05 Jun 2026 12:12:15 GMT

### Minor changes

- Add `grackle agent` command group (list/create/show/edit/delete) for the new minimal Agent entity (#1417).
- Add `agent edit --heartbeat`, `--heartbeat-clear`, `--heartbeat-rules`, `--heartbeat-rules-file`, `--heartbeat-pause`, `--heartbeat-resume` flags and a heartbeat block in `agent show` (#1438).
- Scope tunnel registry into TunnelRegistry class for per-instance isolation

### Patches

- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Extract shared gRPC request validation helpers (require-helpers) to consolidate repeated field and entity-lookup validation across handler files
- Centralize ConnectError handling into grackleError.ts utility
- Add @grackle-ai/test-utils package with type-safe mock infrastructure derived from DatabaseStores interfaces

## 0.174.0
Fri, 05 Jun 2026 07:17:48 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Extract shared retryWithBackoff utility in adapter-sdk, consolidating duplicate retry loops across adapters
- Centralize timestamp creation with serverTimestamp() utility

## 0.173.0
Fri, 05 Jun 2026 06:31:53 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Replace hardcoded plugin registration with self-registering PluginRegistry
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Add GrackleError base class with domain subclasses and server-side ConnectRPC interceptor for unified error handling
- Replace hardcoded runtime registration in PowerLine with catalog-driven dynamic loading
- Split ahp-handlers.ts into focused modules for maintainability
- Add store interfaces, DatabaseStores type, and DI registry for dependency injection foundation
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix Copilot runtime tool permissions by passing --yolo to the CLI for headless auto-approval

## 0.162.0
Wed, 03 Jun 2026 19:05:25 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Revert EventStream virtualization (react-virtuoso) due to regressions
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix duplicate user messages in chat by rendering turn_started as the canonical user input instead of emitting a separate USER_INPUT event
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Add rich mock data for Refactor Bot and Security Auditor agents in demo mode

## 0.158.0
Wed, 03 Jun 2026 08:02:21 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix N+1 queries in resolveAncestorEnvironmentId — batch ancestor + session lookups

## 0.157.0
Wed, 03 Jun 2026 07:32:07 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix AgentChatTab to render ChatInput for suspended sessions so the reconnect button remains reachable

## 0.156.0
Wed, 03 Jun 2026 07:01:24 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Unify PluginContext interface: plugin-sdk is now the single source of truth for PluginContext, Disposable, and SubscriberFactory
- Fix provision-clear timers firing after component unmount in useEnvironments
- Fix N+1 queries and O(N²) scan in task-store hot paths (areDependenciesMet, getUnblockedTasks, getAncestors, reparentTask)

## 0.155.0
Wed, 03 Jun 2026 05:48:30 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix debounce/timer leaks on disconnect in useTasks and useEnvironments hooks
- Add circular task dependency validation to prevent deadlocks
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix || numeric defaults to use ?? so --limit 0 is honored instead of silently overridden

## 0.152.0
Tue, 02 Jun 2026 23:22:10 GMT
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Memoize useGrackleSocket return and domain hook objects to prevent cascading re-renders
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).
- Fix forward tunnel process leak when reverse tunnel fails to open (SSH and Codespace adapters)
- Add missing database indexes on tasks.parent_task_id, sessions.env_id, tasks.workspace_id, sessions.parent_session_id, and schedules.task_id for query performance

## 0.149.0
Tue, 02 Jun 2026 15:35:17 GMT
...
## 0.148.0
Tue, 02 Jun 2026 14:19:55 GMT

### Minor changes

- Add `grackle agent` command group (list/create/show/edit/delete) for the new minimal Agent entity (#1417).
- Live agent view: tabbed agent page (Chat/Sessions/Schedules/Settings), nav restructuring (Personas + Environments to fleet rail), per-agent settings hub with heartbeat controls
- Add `agent edit --heartbeat`, `--heartbeat-clear`, `--heartbeat-rules`, `--heartbeat-rules-file`, `--heartbeat-pause`, `--heartbeat-resume` flags and a heartbeat block in `agent show` (#1438).
...
- `grackle agent create` now requires `--environment <id>`; `agent show` displays the agent's home environment (#1418).
- Unify spawn-config resolution into a single declared cascade/resolver (#1427)
- Lockstep version bump for the `rawPrompt` option on `startTaskSession` (#1442, prereq for #1438).

## 0.146.0
Tue, 02 Jun 2026 03:13:31 GMT
