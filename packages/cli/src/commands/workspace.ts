...
export function registerWorkspaceCommands(program: Command): void {
    ...
    .action(async (opts: { env?: string }) => {
      ...
      for (const p of res.workspaces) {
        table.push([
          p.id,
          p.name,
          p.linkedEnvironmentIds.join(", ") || "-",
          p.useWorktrees ? "enabled" : "disabled",
          workspaceStatusToString(p.status),
          p.createdAt,
        ]);
      }
  ...
  workspace
    ...
    .requiredOption(
      "--env <env-id>",
      "Environment ID (required, auto-linked as the initial environment)",
    )
    ...
    .option(
      ...
      "Working directory / repo root on the environment (e.g. /workspaces/my-repo)",
    )
    ...
    .option(
      ...
      "Aggregate cost cap in millicents ($0.00001 units); 0 = unlimited",
      parseInt,
    )
    .action(
      async (
        ...
        console.log(
          `Created workspace: ${p.id} (${p.name}) [worktrees: ${p.useWorktrees ? "enabled" : "disabled"}]`,
        );
      },
    );
  ...
  workspace
    ...
    .action(async (id: string) => {
      ...
      table.push(
        ...
        {
          "Linked Envs":
            p.linkedEnvironmentIds.length > 0 ? p.linkedEnvironmentIds.join(", ") : "none",
        },
  ...
  workspace
    ...
    .action(
      async (
        ...
        console.log(
          `Updated workspace: ${p.id} (${p.name}) [worktrees: ${p.useWorktrees ? "enabled" : "disabled"}]`,
        );
      },
    );
