...
export function buildTaskSystemContext(title: string, description: string, reviewNotes: string, canDecompose?: boolean): string {
  ...
  sections.push(
    ...
    `    First, write the GraphQL query to a temp file (this avoids shell escaping issues with \\$ in queries):`,
    `    \`\`\``,
    `    cat > /tmp/copilot-threads.graphql << 'GRAPHQL'`,
    `    query($owner: String!, $repo: String!, $pr: Int!) {`,
    `      repository(owner: $owner, name: $repo) {`,
    `        pullRequest(number: $pr) {`,
    `          reviewThreads(first: 100) {`,
    `            nodes { id isResolved isOutdated path line comments(last: 1) { nodes { id author { login } body } } }`,
    `          }`,
    `        }`,
    `      }`,
    `    }`,
    ...
    `    - Resolve the thread (use GraphQL mutation — write to file and use \`-F query=@file\` to avoid escaping issues)`,
