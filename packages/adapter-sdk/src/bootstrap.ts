...
export async function writeRemoteEnvFile(
  ...
  await executor.exec(`chmod 600 ${REMOTE_POWERLINE_DIRECTORY}/.env.sh`, {
    timeout: REMOTE_EXEC_DEFAULT_TIMEOUT_MS,
  });
}
...
export async function startRemotePowerLine(
  ...
  const entryPoint = devMode ? "dist/index.js" : "node_modules/@grackle-ai/powerline/dist/index.js";
  ...
  const sourceEnv = envFileContent ? `. ${REMOTE_POWERLINE_DIRECTORY}/.env.sh && ` : "";
  parts.push(
  ...
  try {
    const stdout = await executor.exec(`bash -c '${shellEscape(compoundScript)}'`, {
      timeout: REMOTE_EXEC_DEFAULT_TIMEOUT_MS,
    });
...
export async function* bootstrapPowerLine(
  ...
  yield { stage: "bootstrapping", message: "Checking Node.js on remote host...", progress: 0.1 };
  try {
    const nodeVersionOutput = await executor.exec("node --version", {
  ...
  if (devMode) {
    ...
    yield {
      stage: "bootstrapping",
      message: "Installing dependencies on remote host...",
      progress: 0.4,
    };
    ...
    for (const [name, dir] of workspacePackages) {
      ...
      yield {
        stage: "bootstrapping",
        message: `Copying @grackle-ai/${name}...`,
        progress: name === "common" ? 0.57 : 0.59,
      };
    ...
    yield { stage: "bootstrapping", message: "Creating remote directories...", progress: 0.2 };
    await executor.exec(`mkdir -p ${REMOTE_POWERLINE_DIRECTORY}`, {
      timeout: REMOTE_EXEC_DEFAULT_TIMEOUT_MS,
    });

    yield {
      stage: "bootstrapping",
      message: `Installing @grackle-ai/powerline@${version}...`,
      progress: 0.25,
    };
    await executor.exec(
  ...
  try {
    ...
    logger.warn(
      { err },
      "Failed to configure git credential helper (agents may be unable to push)",
    );
  }
  ...
  if (defaultRuntime) {
    ...
    if (runtimeManifest) {
      ...
      try {
        ...
        await executor.exec(`mkdir -p ${runtimeDir}`, { timeout: REMOTE_EXEC_DEFAULT_TIMEOUT_MS });
        ...
        if (!runtimeUpToDate) {
          yield {
            stage: "bootstrapping",
            message: `Installing ${defaultRuntime} runtime...`,
            progress: 0.57,
          };
          ...
          logger.info(
            { defaultRuntime, runtimeDir },
            "Default runtime pre-installed on remote host",
          );
        } else {
          logger.info(
            { defaultRuntime, runtimeDir },
            "Default runtime already up to date on remote host; skipping pre-install",
          );
        }
      } catch (err) {
        logger.warn(
          { defaultRuntime, err },
          "Failed to pre-install default runtime (will be installed on first spawn)",
        );
      }
    }
  }
  ...
  yield {
    stage: "bootstrapping",
    message: "Stopping any existing PowerLine process...",
    progress: 0.6,
  };
  try {
