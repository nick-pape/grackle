...
describe("queryDomainEvents handler", () => {
  ...
  it("maps non-empty request filters to the store query", async () => {
    await handler(
      create(grackle.QueryDomainEventsRequestSchema, {
        beforeId: "01Z",
        type: "task.created",
...
describe("getStreamTranscript handler", () => {
  ...
  it("maps request to the store query (stream id + before cursor + limit)", async () => {
    await transcriptHandler(
      create(grackle.GetStreamTranscriptRequestSchema, {
        streamId: "stream-1",
        beforeSeq: "01Z",
        limit: 25,
      }),
    );
    expect(streamStoreMock).toHaveBeenCalledWith({
      streamId: "stream-1",
      beforeSeq: "01Z",
      limit: 25,
    });
  ...
  it("omits an empty before cursor and a zero limit", async () => {
    await transcriptHandler(
      create(grackle.GetStreamTranscriptRequestSchema, { streamId: "stream-1" }),
    );
  ...
  it("maps rows to proto StreamMessageEvent", async () => {
    ...
    const res = await transcriptHandler(
      create(grackle.GetStreamTranscriptRequestSchema, { streamId: "stream-1" }),
    );
...
describe("getSessionActions handler", () => {
  ...
  it("maps request to the store query (session id + from cursor + limit)", async () => {
    await sessionActionsHandler(
      create(grackle.GetSessionActionsRequestSchema, {
        sessionId: "sess-1",
        fromSeq: "01Z",
        limit: 25,
      }),
    );
    expect(sessionStoreMock).toHaveBeenCalledWith({
      sessionId: "sess-1",
      fromSeq: "01Z",
      limit: 25,
    });
  ...
  it("omits an empty from cursor and a zero limit", async () => {
    await sessionActionsHandler(
      create(grackle.GetSessionActionsRequestSchema, { sessionId: "sess-1" }),
    );
  ...
  it("maps rows to proto SessionAction (oldest first / replay order)", async () => {
    ...
    const res = await sessionActionsHandler(
      create(grackle.GetSessionActionsRequestSchema, { sessionId: "sess-1" }),
    );
