...
export function toPersonaModel(row: PersonaRow): PersonaModel {
  return {
    ...
    toolConfig: parseToolConfig(row.toolConfig),
    runtime: row.runtime,
    model: row.model,
    ...
    type: row.type,
    script: row.script,
    allowedMcpTools: parseStringArray(row.allowedMcpTools),
    createdAt: row.createdAt,
    updatedAt: row.updatedAt,
  };
}

function parseToolConfig(json: string): ToolConfigSpec | undefined {
  if (!json) {
    return undefined;
  }
  let parsed: unknown;
...
function parseMcpServers(json: string): McpServerSpec[] {
  if (!json) {
    return [];
  }
  let parsed: unknown;
