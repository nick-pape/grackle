...
vi.mock("./credential-bundle.js", () => ({ buildProviderTokenBundle: vi.fn(async () => ({ tokens: [] })) }));
...
interface TokenItemLike { name: string; type: string; envVar: string; filePath: string; value: string; }

function tok(name: string, type: string, envVar: string): TokenItemLike {
  return { name, type, envVar, filePath: type === "file" ? "~/x" : "", value: "v" };
}

function mockConn(): { client: { authenticate: ReturnType<typeof vi.fn> }; environmentId: string; port: number } {
  return { client: { authenticate: vi.fn().mockResolvedValue({}) }, environmentId: "env-1", port: 7433 };
}
...
describe("authenticateForRuntime", () => {
  ...
  it("delivers the combined stored + provider bundle via authenticate", async () => {
    ...
    vi.mocked(tokenStore.getBundle).mockReturnValue({ tokens: [tok("u", "env_var", "U")] } as never);
    vi.mocked(buildProviderTokenBundle).mockResolvedValue({ tokens: [tok("p", "env_var", "P")] } as never);

    await authenticateForRuntime("env-1", "claude-code");

    expect(conn.client.authenticate).toHaveBeenCalledTimes(1);
    const arg = conn.client.authenticate.mock.calls[0][0] as { provider: string; tokens: TokenItemLike[] };
  ...
  it("excludes file tokens when requested", async () => {
    ...
    vi.mocked(buildProviderTokenBundle).mockResolvedValue({ tokens: [tok("e", "env_var", "E")] } as never);
  ...
  it("swallows delivery errors (best-effort, never throws)", async () => {
    ...
    vi.mocked(adapterManager.getConnection).mockReturnValue(conn as never);
    vi.mocked(tokenStore.getBundle).mockReturnValue({ tokens: [tok("e", "env_var", "E")] } as never);
    await expect(authenticateForRuntime("env-1", "claude-code")).resolves.toBeUndefined();
  });
});
