...
import { z } from "zod";
import {
  BUILTIN_COMPONENT_SCHEMAS,
  type BuiltinComponentName,
} from "./builtin-component-schemas.js";
...
> = {
  ...
  SplitButton: {
    ...
    example:
      '<SplitButton label="Save" onClick={() => {}} options={[{ label: "Save as draft", onClick: () => {} }]} />',
  },
  Callout: {
...
export const BUILTIN_COMPONENT_JSON_SCHEMAS: Readonly<Record<BuiltinComponentName, object>> =
  Object.fromEntries(
    (Object.entries(BUILTIN_COMPONENT_SCHEMAS) as [BuiltinComponentName, z.ZodType][]).map(
      ([name, schema]) => [name, z.toJSONSchema(schema)],
    ),
