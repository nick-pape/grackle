...
describe("retryWithBackoff", () => {
  ...
  describe("shouldRetry", () => {
    ...
    it("fails immediately without sleeping when shouldRetry returns false", async () => {
      const op = vi.fn(async () => {
    ...
    it("stops retrying mid-sequence when shouldRetry returns false for a specific error", async () => {
      let calls = 0;
      const op = vi.fn(async () => {
        calls++;
        if (calls === 1) {
