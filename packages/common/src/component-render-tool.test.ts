...
describe("RENDER_TOOL_PREFIX is reserved", () => {
  it("no statically-registered MCP tool name uses the render_ prefix", () => {
    for (const name of ALL_MCP_TOOL_NAMES) {
      expect(name.startsWith(RENDER_TOOL_PREFIX), `${name} must not use the reserved render_ prefix`).toBe(false);
    }
  });
});

describe("selectPromotedRenderTools", () => {
  const comp = (name: string, promoted: boolean, id = name): { id: string; name: string; promoted: boolean } => ({ id, name, promoted });

  it("selects only promoted components, paired with their tool name", () => {
    const out = selectPromotedRenderTools([comp("Chart", true), comp("Draft", false), comp("Table", true)]);
